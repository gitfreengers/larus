...
class SentryServiceProvider extends ServiceProvider {
    ...
    /**
     * Register the puesto provider used by Sentry.
     *
     * @return void
     */
    protected function registerPuestoProvider()
    {
        $this->app['sentry.puesto'] = $this->app->share(function($app)
        {
            $model = $app['config']['cartalyst/sentry::puestos.model'];
            ...
            // Define the user puesto pivot table name to use for relationships.
            if (method_exists($model, 'setUserPuestosPivot'))
            {
                $pivotTable = $app['config']['cartalyst/sentry::user_puestos_pivot_table'];

                forward_static_call_array(
                    array($model, 'setUserPuestosPivot'),
                    array($pivotTable)
                );
            }

            return new PuestoProvider($model);
        });
    }
