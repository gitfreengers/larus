...
class Puesto extends Model implements PuestoInterface {
	 ...
	 * @var string
	 */
	protected static $userPuestosPivot = 'users_puestos';
	...
	{
		$puestosPermissions = $this->getPermissions();
		...
		foreach ($permissions as $permission)
		{
			...
			if ((strlen($permission) > 1) and ends_with($permission, '*'))
			{
				...
				foreach ($puestosPermissions as $puestoPermission => $value)
				{
					...
					if ($checkPermission != $puestoPermission and starts_with($puestoPermission, $checkPermission) and $value == 1)
					{
						$matched = true;
						break;
					}
			...
			elseif ((strlen($permission) > 1) and starts_with($permission, '*'))
			{
				...
				foreach ($puestoPermissions as $puestoPermission => $value)
				{
					...
					if ($checkPermission != $puestoPermission and ends_with($puestoPermission, $checkPermission) and $value == 1)
					{
						$matched = true;
						break;
					}
			...
			else
			{
				...
				foreach ($puestoPermissions as $puestoPermission => $value)
				{
					...
					if ((strlen($puestoPermission) > 1) and ends_with($puestoPermission, '*'))
					{
						$matched = false;

						// Strip the '*' off the end of the permission.
						$checkPuestoPermission = substr($puestoPermission, 0, -1);

						// We will make sure that the merged permission does not
						// exactly match our permission, but starts wtih it.
						if ($checkPuestoPermission != $permission and starts_with($permission, $checkPuestoPermission) and $value == 1)
						{
							$matched = true;
							break;
						}
					...
					elseif ($permission == $puestoPermission and $puestoPermissions[$permission] == 1)
					{
						$matched = true;
						break;
					}
	...
	{
		return $this->belongsToMany(static::$userModel, static::$userPuestosPivot);
	}
	 ...
	 * @return void
	 */
	public static function setUserPuestosPivot($tableName)
	{
		static::$userPuestosPivot = $tableName;
	}
	...
	public function validate()
	{
		...
		{
			throw new PuestoExistsException("A group already exists with name [$name], names must be unique for groups.");
		}
