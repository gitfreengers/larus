...
use Stevemo\Cpanel\Puesto\Form\PuestoForm;
use Stevemo\Cpanel\Puesto\Form\PuestoValidator;
use Stevemo\Cpanel\Puesto\Repo\PuestoRepository;
...
class CpanelServiceProvider extends ServiceProvider {
    ...
    /**
     * Register Puesto binding
     *
     * @author Steve Montambeault
     * @link   http://stevemo.ca
     *
     */
    public function registerPuesto()
    {
        $app = $this->app;

        $app->bind('Stevemo\Cpanel\Puesto\Repo\CpanelPuestoInterface', function($app)
        {
            return new PuestoRepository($app['sentry'], $app['events']);
        });

        $app->bind('Stevemo\Cpanel\Puesto\Form\PuestoFormInterface', function($app)
        {
            return new PuestoForm(
                new PuestoValidator($app['validator'], new MessageBag),
                $app->make('Stevemo\Cpanel\Puesto\Repo\CpanelPuestoInterface')
            );
        });
    }
